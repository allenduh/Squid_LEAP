#!/usr/bin/env python3
"""
bin_trace_viewer_fixed.py
-------------------------
Fast PyQtGraph viewer for binned camera frames (+ trace visualization).

Fixes:
- Places QSlider and labels in a normal QWidget layout (not GraphicsLayoutWidget),
  avoiding the 'geometryChanged' attribute error.

Features:
- Reads batch_*.raw files (uint8) with numpy.memmap
- Vectorized binning (reshape + mean) into float32
- Shows a playable binned image (frame-by-frame)
- Click the image to add/remove ROI traces (up to 8)
- Plots global mean trace
- Scrub with a slider; Space=Play/Pause; C=Clear traces

Example:
python bin_trace_viewer_fixed.py \
...
class Viewer(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        for _ in range(self.max_traces):
            item = pg.PlotCurveItem(skipFiniteCheck=True)
        ...
        # Keep a separate list to manage trace items
        self.trace_items = self.trace_plot.listDataItems()
