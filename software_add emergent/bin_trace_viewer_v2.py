#!/usr/bin/env python3
"""
bin_trace_viewer_v2.py
----------------------
PyQtGraph viewer for binned camera frames with robust compatibility across
PyQtGraph versions (no use of PlotCurveItem.setDownsampling).

Changes vs. previous version:
- Replace PlotCurveItem with PlotDataItem and avoid setDownsampling()
- Optional manual decimation for very long traces
- Fixed a stray parenthesis in toggle_play()

Features:
- Reads batch_*.raw files (uint8) via numpy.memmap
- Vectorized binning (reshape+mean) into float32
- Playable binned image; click to add/remove ROI traces (up to 8)
- Global mean trace plot
- Slider to scrub; Space=Play/Pause; C=Clear traces

Example:
python bin_trace_viewer_v2.py \
...
class Viewer(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        self.global_item = pg.PlotDataItem(gx, gy)
        try:
            self.global_item.setClipToView(True)
        except Exception:
            pass
    ...
    def _update_traces(self):
        # Update ROI traces; decimate to keep plotting fast for long series
