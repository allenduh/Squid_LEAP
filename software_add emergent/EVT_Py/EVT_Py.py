...
from ._eSDK_cffi._eSDK_cffi_wrapper import _EvtContext, _EvtCameraOpenParams, _EvtDeviceList, _EvtDeviceInfo, _EvtCamera, _EvtFrame
...
class EvtColorConvert(IntEnum):
    EVT_CONVERT_NONE = 0
    EVT_COLOR_CONVERT_TO_RGB = 4
    EVT_COLOR_CONVERT_NEARESTNEIGHBOR_RGB = 1
    EVT_COLOR_CONVERT_NEARESTNEIGHBOR_BGR = 12
...
class EvtCamera:
    ...
    def allocate_frame(self, width: int = 0, height: int = 0, pixel_type: EvtPixelFormat = 0) -> EvtFrame:
        return EvtFrame(self._impl.allocate_frame(width, height, pixel_type))
    ...
    # Returns the frame.
    def get_frame(self, timeout: int = -1) -> EvtFrame:
       return EvtFrame(self._impl.get_frame(timeout))
    ...
    # Gets range of an enum parameter as a list
    def get_enum_range(self, param: str) -> list[str]:
        return self._impl.get_enum_range(param)
...
class EvtDeviceInfo:
    def __init__(self, impl: _EvtDeviceInfo):
        ...
        self.mac_address = impl.mac_address
        self.ip_address = impl.ip_address
