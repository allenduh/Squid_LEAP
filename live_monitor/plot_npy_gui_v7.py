...
FIGSIZE        = (22, 11)  # slightly shorter overall window height
...
FAST_SHIFT_STRIDE = 4      # when holding Shift while scanning, decimate time by this stride for speed
...
class StackGUI:
    def __init__(self, stack: np.ndarray, fps: float, rows: int, cols: int, kernel: int):
        ...
        # Grid-move modifier: hold 'm' and use arrows to move the grid (avoid 'g' which toggles Matplotlib grid)
        ...
        self.sc1 = self.ax_img.scatter(xs, ys, s=52, facecolors='none', edgecolors='k', linewidths=1.3)
        self.sc2 = self.ax_img.scatter(xs, ys, s=28, facecolors='none', edgecolors='w', linewidths=1.0)
        ...
        (self.line_over,) = self.ax_over.plot(self.t_all, self.global_mean, lw=0.9, label="Global mean")
        ...
        self.fig.suptitle("Stack Explorer — kernel-averaged traces, fast time scan, 'm'+arrows to move grid", fontsize=15)
    ...
    def _render_help_text(self):
        ...
        self.ax_help.set_axis_off()
        text = (
            "Time window: ←/→ shift (±5), Shift+←/→ (±50, fast subsampled), [ / ] shrink/expand, Z zoom-in x0.5, X zoom-out x2\n"
            "Move grid: HOLD 'm' then use ↑/↓/←/→ (add Shift for larger step)\n"
            ...
            "Layout: top=40%, middle=10%, bottom=50% (HEIGHT_RATIOS); trace rows scaled 1.2x\n"
        )
    ...
    def on_key_press(self, event):
        ...
        if event.key in ('left','right','shift+left','shift+right'):
            ...
            # During Shift, recompute with time decimation for speed (so new time is shown immediately)
...
def main():
    ap = argparse.ArgumentParser(description=".npy (T,Y,X) stack explorer with movable grid, kernel-averaged traces, and fast scanning.")
    ...
    _ = StackGUI(stack, fps=args.fps, rows=args.rows, cols=args.cols, kernel=args.kernel)
