...
def bilinear_grid_from_quad(pts: np.ndarray, rows_v: int, cols_v: int) -> np.ndarray:
    TL, TR, BL, BR = pts[0], pts[1], pts[2], pts[3]
    yy = (np.arange(rows_v) + 0.5) / rows_v
    xx = (np.arange(cols_v) + 0.5) / cols_v
    grid = np.stack(np.meshgrid(yy, xx, indexing="ij"), axis=-1).reshape(-1, 2)
    v = grid[:, 0:1]; u = grid[:, 1:2]
    top = (1 - u) * TL + u * TR
    bottom = (1 - u) * BL + u * BR
    P = (1 - v) * top + v * bottom
    return P

def save_roi_json(json_path: Path, rows_v: int, cols_v: int, centers: np.ndarray, box_size: int):
    polys = []
    h = box_size // 2
    for (x, y) in centers:
        xa, xb = int(x - h), int(x + h)
        ya, yb = int(y - h), int(y + h)
        polys.append([[xa, ya], [xb, ya], [xb, yb], [xa, yb]])
    payload = {
        "rows": int(rows_v),
        "cols": int(cols_v),
        "centers_xy_f": centers.astype(float).tolist(),
        "cell_polygons": polys,
        "box_size": int(box_size),
    }
    json_path.write_text(json.dumps(payload, indent=2))

# --- Interactive editor: preview & drag ROI grid ---

def edit_roi_json_on_image(seed_img: Path, json_path: Path, box_size: int, init_rows: Optional[int]=None, init_cols: Optional[int]=None):
    import matplotlib
    ...
    img = mpimg.imread(str(seed_img))
    H, W = img.shape[:2]

    rows_v, cols_v, centers = None, None, None
    if json_path.exists():
        try:
            r, c, ctrs, _ = load_roi(json_path)
            rows_v, cols_v, centers = r, c, ctrs.astype(float)
        except Exception:
            pass

    if centers is None:
        # Need 4-point selection first
        fig0, ax0 = plt.subplots(figsize=(8, 6))
        ax0.imshow(img, cmap="gray")
        ax0.set_title("Click 4 points: TL, TR, BL, BR. Close when done.")
        pts = []
        def on_click0(event):
            if event.xdata is None or event.ydata is None: return
            if len(pts) >= 4: return
            pts.append([event.xdata, event.ydata])
            ax0.plot(event.xdata, event.ydata, 'rx'); fig0.canvas.draw()
        cid0 = fig0.canvas.mpl_connect('button_press_event', on_click0)
        plt.show()
        fig0.canvas.mpl_disconnect(cid0)
        if len(pts) != 4:
            raise RuntimeError(f"Need 4 points, got {len(pts)}.")
        import tkinter as tk
        from tkinter import simpledialog
        root = tk.Tk(); root.withdraw()
        rr = init_rows or simpledialog.askinteger("ROI Rows", "Number of rows:", parent=root, minvalue=1, maxvalue=10000)
        cc = init_cols or simpledialog.askinteger("ROI Cols", "Number of cols:", parent=root, minvalue=1, maxvalue=10000)
        root.destroy()
        if not rr or not cc:
            raise RuntimeError("Rows/Cols not provided.")
        centers = bilinear_grid_from_quad(np.array(pts, dtype=float), rr, cc)
        rows_v, cols_v = rr, cc

    # Interactive drag
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.imshow(img, cmap="gray")
    ax.set_title("Drag points to adjust grid. Keys: S=save, Q=quit")
    scat = ax.scatter(centers[:,0], centers[:,1], s=10, c='lime', linewidths=0.5, edgecolors='k')

    # draw boxes
    half = box_size // 2
    rects = []
    for (x, y) in centers[:min(200, centers.shape[0])]:  # draw up to 200 boxes for speed
        rect = Rectangle((x-half, y-half), box_size, box_size, fill=False, edgecolor='yellow', linewidth=0.5, alpha=0.7)
    ...
    def on_move(event):
        ...
        if event.xdata is None or event.ydata is None: return
        centers[drag_idx, 0] = np.clip(event.xdata, 0, W-1)
        centers[drag_idx, 1] = np.clip(event.ydata, 0, H-1)
        scat.set_offsets(centers[:, :2])
        # update a few rects (re-draw first N for speed)
        for j, rect in enumerate(rects):
            x, y = centers[j,0], centers[j,1]
            rect.set_xy((x-half, y-half))
        fig.canvas.draw_idle()

    def on_key(event):
        if event.key in ('s', 'S'):
            save_roi_json(json_path, rows_v, cols_v, centers, box_size)
            ax.set_title(f"Saved: {json_path}. Continue editing or press Q to quit."); fig.canvas.draw()
...
def _create_seed_bmp_from_first_raw(in_dir: Path, out_dir: Path, H: int, W: int, N: int) -> Path:
    jobs = list_raw_files(in_dir, max_files=1)
    if not jobs:
        raise RuntimeError("No RAW files found to create a seed image.")
    raw_path = jobs[0][1]
    frame0 = memmap_first_frame(raw_path, H, W, N)
    out_bmp = out_dir / "roi_seed.bmp"
    try:
        from PIL import Image  # type: ignore
        Image.fromarray(frame0).save(out_bmp)
        return out_bmp
    except Exception:
        out_png = out_dir / "roi_seed.png"
        try:
            from PIL import Image  # type: ignore
            Image.fromarray(frame0).save(out_png)
        except Exception:
            tiff.imwrite(str(out_dir / "roi_seed.tif"), frame0)
            return out_dir / "roi_seed.tif"
        return out_png
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(rframe, text="Create seed image from first RAW", command=self._create_roi_seed).pack(side="left")
        ttk.Button(rframe, text="Preview/Edit ROI Grid", command=self._preview_edit_roi).pack(side="left", padx=8)
    ...
    def _create_roi_seed(self):
        try:
            in_dir = Path(self.in_dir.get())
            if not in_dir.exists():
                messagebox.showerror("Missing input", "Pick RAW input folder first."); return
            out_dir = Path(self.roi_json.get()).parent if self.roi_json.get().strip() else in_dir
            out_dir.mkdir(parents=True, exist_ok=True)
            out_path = _create_seed_bmp_from_first_raw(in_dir, out_dir, int(self.height.get()), int(self.width.get()), int(self.frames_per_raw.get()))
            self._log(f"Created seed image: {out_path}")
            messagebox.showinfo("Seed image", f"Seed written:\n{out_path}")
        except Exception as e:
            self._log(f"[ERROR] ROI seed: {e}\n{traceback.format_exc()}")
            messagebox.showerror("Error creating seed", str(e))

    def _preview_edit_roi(self):
        try:
            # choose seed image (BMP/PNG/TIF)
            f = filedialog.askopenfilename(title="Select seed image (bmp/png/tif)", filetypes=[("Images","*.bmp *.png *.tif *.tiff")])
            if not f:
                return
            # choose or confirm JSON path
            j = self.roi_json.get().strip()
            if not j:
                j = str(Path(f).with_name("roi_grid_config.json"))
                self.roi_json.set(j)
            edit_roi_json_on_image(Path(f), Path(j), int(self.box.get()))
            self._log(f"ROI JSON saved/updated: {j}")
            messagebox.showinfo("ROI JSON", f"Saved/updated:\n{j}")
        except Exception as e:
            self._log(f"[ERROR] ROI editor: {e}\n{traceback.format_exc()}")
            messagebox.showerror("ROI editor error", str(e))
    ...
    def _run(self):
        try:
            ...
            if roi_json_path is None:
                messagebox.showwarning("ROI missing", "Please provide ROI JSON or create one with the editor."); return
            cm = CompressionManager(
                ...
                start=int(self.start.get()), end=_safe_int(self.end.get()),
                max_files=_safe_int(self.max_files.get()),
