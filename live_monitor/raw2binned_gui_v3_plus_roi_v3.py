...
import os, sys, json, re, threading, queue, subprocess, traceback, csv
...
class ROIGridUI:
    ...
    def _on_click(self, e):
        ...
        if self.corners is not None:
            ...
            tol = max(6.0, 0.01*max(self.img.shape[0], self.img.shape[1]))
            i = int(np.argmin(dists))
            if dists[i] <= tol:
                self.drag_idx = i
                return
        if len(self._temp_clicks) < 4:
            self._temp_clicks.append((x, y)); self._draw_temp()
            if len(self._temp_clicks)==4:
    ...
    def _draw_temp(self):
        ...
        for (x,y) in self._temp_clicks:
            dot, = self.ax.plot(x,y,'o',ms=5,mec='cyan',mfc='none',ls='', zorder=6)
    ...
    def _order_corners(points: List[Tuple[float,float]]):
        ...
        pts = pts[np.argsort(ang)]  # clockwise
    ...
    def _redraw(self, lite=False):
        ...
        l, = self.ax.plot(xs,ys,c='cyan',lw=1,alpha=0.9, zorder=4); self._artists.append(('grid', l))
        ...
        if self.show_boxes:
            ...
            for r in range(self.rows):
                ...
                for c in range(self.cols):
                    ...
                    l, = self.ax.plot(xs,ys,c='cyan',lw=0.8,alpha=0.85, zorder=4)
    ...
    def _install_handles(self):
        ...
        for i,(x,y) in enumerate(self.corners):
            circ = Circle((x,y), radius=8, facecolor='none', edgecolor='yellow', lw=1.5, zorder=7)
            self.ax.add_patch(circ); self.handles.append(circ)
            # label the corner
            self.ax.text(x+6, y-6, f"{['TL','TR','BR','BL'][i]}", color='yellow', fontsize=8, zorder=8)
...
# ============================== v3 binning pipeline with adaptive prefetch ==============================
...
class Prefetcher(threading.Thread):
    daemon = True
    def __init__(self, jobs: List[Tuple[int, Path]], H: int, W: int, N_default: int,
                 start_idx: int, end_idx: Optional[int], batch: int, q: "queue.Queue"):
        super().__init__()
        self.jobs = jobs
        self.H = int(H); self.W = int(W); self.N_default = int(N_default)
        self.start_idx = start_idx   # avoid Thread.start() name clash
        self.end_idx = end_idx
        self.batch = batch
        self.q = q

    def _map_file(self, raw_path: Path) -> Tuple[np.memmap, bool, int]:
        """Return (memmap, transposed?, N). If first few frames are all zero, try transpose layout."""
        # Decide N per-file from size when possible
        N_guess = infer_frames_per_raw_by_size(raw_path, self.H, self.W) or self.N_default
        need = int(self.H) * int(self.W) * int(N_guess)
        sz = raw_path.stat().st_size
        if sz < need and self.N_default != N_guess:
            # fall back to default N if size didn't match inferred
            N_guess = self.N_default
            need = int(self.H) * int(self.W) * int(N_guess)
        # Try (N,H,W)
        try:
            mm = np.memmap(raw_path, mode="r", dtype=np.uint8, shape=(N_guess, self.H, self.W))
            # probe first 3 frames
            k = min(3, N_guess)
            means = [float(np.mean(mm[i])) for i in range(k)]
            if any(m > 0.5 for m in means):  # non-zero
                return mm, False, N_guess
            # else try transposed
            del mm
        except Exception:
            mm = None  # noqa
        try:
            mm2 = np.memmap(raw_path, mode="r", dtype=np.uint8, shape=(N_guess, self.W, self.H))
            k = min(3, N_guess)
            means = [float(np.mean(mm2[i])) for i in range(k)]
            if any(m > 0.5 for m in means):
                return mm2, True, N_guess
            # both layouts look zero → return first mapping and let consumer handle
            return mm2, True, N_guess
        except Exception:
            # final attempt: map original even if means were zero
            mm3 = np.memmap(raw_path, mode="r", dtype=np.uint8, shape=(N_guess, self.H, self.W))
            return mm3, False, N_guess
    ...
    def run(self):
        ...
        for _, rp in self.jobs:
            try:
                mm, transposed, N = self._map_file(rp)
            except Exception as e:
                print(f"[SKIP] {rp.name}: map failed: {e}")
                continue
            try:
                for i in range(N):
                    if idx_global < self.start_idx:
                        idx_global += 1; continue
                    if self.end_idx is not None and self.end_idx >= 0 and idx_global >= self.end_idx:
                        if buf: self.q.put((seq_start, np.stack(buf, 0)))
                        self.q.put(None); del mm; return
                    fr = mm[i] if not transposed else mm[i].T
                    if not buf: seq_start = idx_global
                    buf.append(np.asarray(fr, copy=False)); idx_global += 1
...
# ============================== diagnostics (validate RAWs) ==============================
from PIL import Image

def validate_raws(in_dir: Path, H: int, W: int, frames_hint: int, out_dir: Path, max_files: int = 8, frames_per_file_probe: int = 6):
    jobs = list_raw_files(in_dir, max_files=max_files)
    if not jobs: raise RuntimeError("No RAW files to validate.")
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = out_dir / "raw_validation.csv"
    contact_path = out_dir / "raw_probe_contact.bmp"

    thumbs = []
    with open(csv_path, "w", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(["file","layout","N","frame_idx","mean","min","max","nonzero"])
        for _, rp in jobs:
            N_guess = infer_frames_per_raw_by_size(rp, H, W) or frames_hint
            # (H,W)
            try:
                mm = np.memmap(rp, mode="r", dtype=np.uint8, shape=(N_guess, H, W))
                for i in range(min(frames_per_file_probe, N_guess)):
                    fr = np.array(mm[i], copy=True)
                    nz = int(np.count_nonzero(fr))
                    wr.writerow([rp.name,"H,W",N_guess,i,float(fr.mean()), int(fr.min()), int(fr.max()), nz])
                    thumbs.append(Image.fromarray(fr).resize((W//4, H//4)))
                del mm
            except Exception:
                pass
            # (W,H) transposed
            try:
                mm = np.memmap(rp, mode="r", dtype=np.uint8, shape=(N_guess, W, H))
                for i in range(min(frames_per_file_probe, N_guess)):
                    fr = np.array(mm[i], copy=True).T
                    nz = int(np.count_nonzero(fr))
                    wr.writerow([rp.name,"W,H->T",N_guess,i,float(fr.mean()), int(fr.min()), int(fr.max()), nz])
                    thumbs.append(Image.fromarray(fr).resize((W//4, H//4)))
                del mm
            except Exception:
                pass
    ...
    if thumbs:
        # tile thumbs into contact sheet
        cols = 6
        rows = (len(thumbs)+cols-1)//cols
        tw = thumbs[0].width; th = thumbs[0].height
        sheet = Image.new("L", (cols*tw, rows*th), color=0)
        for i,im in enumerate(thumbs):
            x = (i % cols)*tw; y = (i // cols)*th
            sheet.paste(im, (x,y))
        sheet.save(contact_path)

    return csv_path, contact_path
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("RAW → (Binned TIFF + NPZ) [v3 + ROI v3]")
    ...
    def _build_ui(self):
        ...
        ttk.Button(rframe, text="Open ROI selector (from RAW)", command=self._open_roi_selector_from_raw).pack(side="left", padx=(0,8))
        ttk.Button(rframe, text="Validate RAWs (CSV + contact BMP)", command=self._validate_raws).pack(side="left")
    ...
    def _open_roi_selector_from_raw(self):
        try:
            ...
            # Build robust seed and SAVE BMP next to JSON (as you requested)
            img_u8 = _robust_seed_from_raw(in_dir, int(self.height.get()), int(self.width.get()), int(self.frames_per_raw.get()))
            seed_bmp = out_dir / "roi_seed.bmp"
            from PIL import Image
            Image.fromarray(img_u8).save(seed_bmp)
            self._log(f"Saved preview BMP: {seed_bmp}")
    ...
    def _validate_raws(self):
        try:
            in_dir = Path(self.in_dir.get())
            if not in_dir.exists():
                messagebox.showerror("Missing input", "Pick RAW input folder first."); return
            if self.roi_json.get().strip():
                out_dir = Path(self.roi_json.get()).parent
            else:
                out_dir = in_dir
            csv_path, contact_path = validate_raws(
                in_dir, int(self.height.get()), int(self.width.get()), int(self.frames_per_raw.get()),
                out_dir, max_files=8, frames_per_file_probe=6
            )
            self._log(f"[VALIDATE] CSV: {csv_path}")
            if contact_path.exists():
                self._log(f"[VALIDATE] Contact BMP: {contact_path}")
            messagebox.showinfo("Validation ready", f"CSV: {csv_path}\nContact BMP: {contact_path if contact_path.exists() else 'n/a'}")
        except Exception as e:
            self._log(f"[ERROR] Validate: {e}\n{traceback.format_exc()}")
