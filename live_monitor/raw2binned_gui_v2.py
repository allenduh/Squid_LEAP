...
class CompressionManager:
    ...
    box_size: int = 32
    in_dir: Path = Path(".")
    exp_dir: Path = Path(".")     # folder containing roi_grid_config.json (ROI folder) + where outputs go
...
def _create_seed_bmp_from_first_raw(in_dir: Path, roi_dir: Path, H: int, W: int, N: int) -> Path:
    ...
    out_bmp = roi_dir / "roi_seed.bmp"
    try:
        from PIL import Image  # type: ignore
        Image.fromarray(frame0).save(out_bmp)
        return out_bmp
    except Exception:
        out_png = roi_dir / "roi_seed.png"
        try:
            from PIL import Image  # type: ignore
            Image.fromarray(frame0).save(out_png)
        except Exception:
            tiff.imwrite(str(roi_dir / "roi_seed.tif"), frame0)
            return roi_dir / "roi_seed.tif"
        return out_png
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Label(frm, text="ROI folder (roi_grid_config.json)").grid(row=1, column=0, sticky="e")
        ...
        ttk.Button(rframe, text="Open previous ROI selector", command=self._open_roi_selector).pack(side="left")
        ttk.Button(rframe, text="Create ROI seed from first RAW", command=self._create_roi_seed).pack(side="left", padx=8)
        ...
        ttk.Label(r, text="Max files (for quick test)").grid(row=1, column=0, sticky="e"); ttk.Entry(r, textvariable=self.max_files, width=8).grid(row=1, column=1, sticky="w")
        ttk.Label(r, text="Batch (frames per chunk)").grid(row=1, column=2, sticky="e"); ttk.Entry(r, textvariable=self.batch, width=8).grid(row=1, column=3, sticky="w")
        ttk.Label(r, text="Prefetch batches (I/O queue depth)").grid(row=2, column=0, sticky="e"); ttk.Entry(r, textvariable=self.prefetch, width=8).grid(row=2, column=1, sticky="w")
    ...
    def _pick_exp(self):
        d = filedialog.askdirectory(title="Select ROI folder (roi_grid_config.json)")
    ...
    def _open_roi_selector(self):
        ...
        messagebox.showinfo("Not found", "Could not find 'roi_select_gui.py'. Place it in the ROI folder or current directory.")

    def _create_roi_seed(self):
        try:
            in_dir = Path(self.in_dir.get())
            roi_dir = Path(self.exp_dir.get())
            if not in_dir.exists():
                messagebox.showerror("Missing input", "Pick RAW input folder first."); return
            if not roi_dir.exists():
                messagebox.showerror("Missing ROI folder", "Pick ROI folder first."); return
            out_path = _create_seed_bmp_from_first_raw(in_dir, roi_dir, int(self.height.get()), int(self.width.get()), int(self.frames_per_raw.get()))
            self._log(f"Created ROI seed image: {out_path}")
            messagebox.showinfo("ROI seed created", f"Seed image written:\n{out_path}")
        except Exception as e:
            self._log(f"[ERROR] ROI seed: {e}")
            messagebox.showerror("Error creating ROI seed", str(e))
    ...
    def _run(self):
        ...
        if not cm.exp_dir.exists():
            messagebox.showerror("Missing ROI folder", f"ROI folder not found: {cm.exp_dir}"); return
        if not cm.roi_config_path().exists():
            self._log("ROI json not found—use 'Create ROI seed' then 'Open previous ROI selector' to create it.")
            messagebox.showwarning("ROI json missing", f"ROI json not found at {cm.roi_config_path()}.\nUse the buttons above to create/select ROI first.")
            return

        self._log(f"Starting… in={cm.in_dir} ROI={cm.exp_dir} tiff_mode={cm.tiff_mode} box={cm.box_size} batch={cm.batch}")
        def _run_bg():
            try:
