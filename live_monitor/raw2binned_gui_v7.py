...
def create_seed_bmp(in_dir: Path, out_dir: Path, H: int, W: int, N: int) -> Path:
    ...
    frame0 = auto_contrast_u8(frame0)  # <-- not black
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(rframe, text="Create BMP for old ROI selector", command=self._create_bmp_for_old_selector).pack(side="left")
        ttk.Button(rframe, text="Launch old ROI selector", command=self._launch_old_selector).pack(side="left", padx=8)

        sframe = ttk.Frame(frm); sframe.grid(row=3, column=0, columnspan=3, sticky="we", **pad)
        ttk.Label(sframe, text="Path to roi_select_gui.py").pack(side="left")
        ttk.Entry(sframe, textvariable=self.selector_path, width=50).pack(side="left", expand=True, fill="x", padx=4)
        ttk.Button(sframe, text="Browse…", command=self._pick_selector_path).pack(side="left")
        ttk.Button(sframe, text="Save", command=self._persist_state).pack(side="left", padx=4)
    ...
    def _create_bmp_for_old_selector(self):
        try:
            ...
            if self.roi_json.get().strip():
                ...
                d = filedialog.askdirectory(title="Select folder where roi_select_gui.py expects BMP/JSON")
                if not d: return
                roi_dir = Path(d)
                self.roi_json.set(str(roi_dir / "roi_grid_config.json"))
            roi_dir.mkdir(parents=True, exist_ok=True)
            out_path = create_seed_bmp(in_dir, roi_dir, int(self.height.get()), int(self.width.get()), int(self.frames_per_raw.get()))
            self._log(f"Created seed image (auto-contrast) for old ROI selector: {out_path}")
            messagebox.showinfo("Seed image", f"Seed written:\n{out_path}")
        except Exception as e:
            self._log(f"[ERROR] Create BMP: {e}\n{traceback.format_exc()}")
            messagebox.showerror("Error creating BMP", str(e))

    def _launch_old_selector(self):
        try:
            # Resolve ROI folder
            if self.roi_json.get().strip():
                roi_dir = Path(self.roi_json.get()).parent
            else:
                d = filedialog.askdirectory(title="Select folder for old ROI selector")
                if not d: return
                roi_dir = Path(d)
                self.roi_json.set(str(roi_dir / "roi_grid_config.json"))

            candidates = []
            if self.selector_path.get().strip():
                cand = Path(self.selector_path.get())
                if cand.exists(): candidates.append(cand)
            candidates.append(roi_dir / "roi_select_gui.py")
            candidates.append(Path.cwd() / "roi_select_gui.py")
            candidates.append(Path.home() / "Documents/Github/Squid_LEAP/live_monitor/roi_select_gui.py")

            for c in candidates:
                if c.exists():
                    self._log(f"Launching ROI selector: {c}")
                    try:
                        subprocess.Popen([sys.executable, str(c), str(roi_dir)], cwd=str(c.parent))
                    except Exception:
                        subprocess.Popen([sys.executable, str(c)], cwd=str(c.parent))
                    return
            messagebox.showinfo("Not found", "Could not find 'roi_select_gui.py'. Use the Browse button to set its path, then click Save.")
        except Exception as e:
            self._log(f"[ERROR] Launch ROI selector: {e}\n{traceback.format_exc()}")
            messagebox.showerror("Error launching ROI selector", str(e))
    ...
    def _run(self):
        try:
            ...
            if roi_json_path is None:
                messagebox.showwarning("ROI missing", "Please provide ROI JSON (use the old selector to create it)."); return
