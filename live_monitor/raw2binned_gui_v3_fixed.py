...
class Prefetcher(threading.Thread):
    ...
    def __init__(self, jobs: List[Tuple[int, Path]], H: int, W: int, N: int,
        ...
        # Renamed to avoid shadowing Thread.start()
...
def run_pipeline(cm: CompressionManager, log=lambda *a, **k: None):
    ...
    # Pass cm.start/cm.end but use the renamed fields in Prefetcher
...
def bilinear_grid_from_quad(pts: np.ndarray, rows: int, cols: int) -> np.ndarray:
    """pts: (4,2) in order TL, TR, BL, BR -> returns (rows*cols, 2) centers via bilinear interpolation."""
    TL, TR, BL, BR = pts[0], pts[1], pts[2], pts[3]
    yy = (np.arange(rows) + 0.5) / rows
    xx = (np.arange(cols) + 0.5) / cols
    grid = np.stack(np.meshgrid(yy, xx, indexing="ij"), axis=-1).reshape(-1, 2)  # (rows*cols, 2) (v,u)
...
def save_roi_json(json_path: Path, rows: int, cols: int, centers: np.ndarray, box_size: int):
    polys = []
    h = box_size // 2
    for (x, y) in centers:
...
def _pick_points_and_make_json(seed_img: Path, json_path: Path, box_size: int) -> None:
    import matplotlib
    ...
    ax.set_title("Click 4 points in order: TL, TR, BL, BR. Close window when done.")
    pts = []
    ...
    rows = simpledialog.askinteger("ROI Rows", "Number of rows:", parent=root, minvalue=1, maxvalue=10000)
    cols = simpledialog.askinteger("ROI Cols", "Number of cols:", parent=root, minvalue=1, maxvalue=10000)
    root.destroy()
    if not rows or not cols:
        raise RuntimeError("Rows/Cols not provided.")

    centers = bilinear_grid_from_quad(np.array(pts, dtype=float), rows, cols)
    save_roi_json(json_path, rows, cols, centers, box_size)

class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("RAW → (Binned TIFF + NPZ) [v3-fixed]")
    ...
    def _build_ui(self):
        ...
        ttk.Button(rframe, text="Create ROI JSON (4‑point)", command=self._create_roi_json_from_seed).pack(side="left", padx=8)
    ...
    def _create_roi_json_from_seed(self):
        from tkinter import filedialog, messagebox
        try:
            f = filedialog.askopenfilename(title="Select seed image (bmp/png/tif)", filetypes=[("Images","*.bmp *.png *.tif *.tiff")])
            if not f:
                return
            default_json = str(Path(f).with_name("roi_grid_config.json"))
            j = filedialog.asksaveasfilename(title="Save ROI JSON as", defaultextension=".json", initialfile=Path(default_json).name)
            if not j:
                return
            _pick_points_and_make_json(Path(f), Path(j), int(self.box.get()))
            self.roi_json.set(j)
            self._log(f"ROI JSON saved: {j}")
            messagebox.showinfo("ROI JSON", f"Saved:\n{j}")
        except Exception as e:
            self._log(f"[ERROR] ROI JSON: {e}")
            messagebox.showerror("Error creating ROI JSON", str(e))
    ...
    def _run(self):
        ...
        try:
            ...
            if roi_json_path is None:
                messagebox.showwarning("ROI missing", "Please provide ROI JSON or create one with the 4‑point tool."); return
