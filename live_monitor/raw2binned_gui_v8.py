...
def infer_frames_per_raw_by_size(raw_path: Path, H: int, W: int) -> Optional[int]:
    # returns N if size is an exact multiple of H*W, else None
    bpp = 1  # uint8
    frame_bytes = H * W * bpp
...
def read_first_nonzero_frame(raw_path: Path, H: int, W: int, frames_hint: Optional[int]=None, max_scan:int=50) -> Optional[np.ndarray]:
    """Try hard to get a non-zero frame for preview without relying strictly on frames_per_raw."""
    frame_bytes = H * W
    N = frames_hint or infer_frames_per_raw_by_size(raw_path, H, W) or 0
    # Fall back: just read first frame_bytes
    ...
    if len(data) >= frame_bytes:
        ...
        if np.any(fr):
            return fr
    # If we have a plausible N, scan up to max_scan frames
    if N > 0:
        try:
            ...
            pass
    # Try transposed shape (some recordings swap H/W)
...
def create_seed_bmp(in_dir: Path, out_dir: Path, H: int, W: int, N_hint: Optional[int]) -> Path:
    jobs = list_raw_files(in_dir, max_files=5)
    if not jobs:
        raise RuntimeError("No RAW files found to create a seed image.")
    # find first nonzero frame from first few raws
    fr = None; src = None
    for _, rp in jobs:
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        ttk.Button(rframe, text="Launch old ROI selector (BMP)", command=self._launch_old_selector_bmp).pack(side="left", padx=8)

        sframe = ttk.LabelFrame(frm, text="ROI Selector Script")
        sframe.grid(row=3, column=0, columnspan=3, sticky="we", **pad)
    ...
    def _create_bmp_for_old_selector(self):
        try:
            ...
            self._log(f"Created seed image for old ROI selector: {out_path}")
    ...
    def _launch_old_selector_bmp(self):
        try:
            # choose a BMP/PNG/TIF directly and pass it to your ROI selector
            if self.selector_path.get().strip():
                ...
                if not sf: return
                script = Path(sf)
                self.selector_path.set(sf); self._persist_state()

            bmp = filedialog.askopenfilename(title="Select seed image (bmp/png/tif)", filetypes=[("Images","*.bmp *.png *.tif *.tiff")])
            if not bmp:
                return

            self._log(f"Launching ROI selector with image: {script}  {bmp}")
            # Try passing BMP path as arg; if script doesn't accept, it will still launch cwd-based
            try:
                subprocess.Popen([sys.executable, str(script), str(bmp)], cwd=str(script.parent))
            except Exception:
    ...
    def _run(self):
        try:
            ...
            if roi_json_path is None:
                messagebox.showwarning("ROI missing", "Please provide ROI JSON (use the ROI selector to create it)."); return
