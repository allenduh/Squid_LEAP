...
class Prefetcher(threading.Thread):
    ...
    def __init__(self, jobs: List[Tuple[int, Path]], H: int, W: int, N: int,
        ...
        # IMPORTANT: avoid masking Thread.start()
...
def run_pipeline(cm: CompressionManager, log=lambda *a, **k: None):
    ...
    pf.start()  # now calls threading.Thread.start()
...
class App(tk.Tk):
    ...
    def _create_bmp_for_old_selector(self):
        try:
            ...
            # Determine ROI folder (where the old selector expects the BMP and JSON)
            if self.roi_json.get().strip():
                ...
                if not d: return
                roi_dir = Path(d)
                # Also fill the JSON path if not set
    ...
    def _launch_old_selector(self):
        try:
            # Prefer ROI json path's folder; otherwise ask
            if self.roi_json.get().strip():
                roi_dir = Path(self.roi_json.get()).parent
            else:
                ...
                roi_dir = Path(d)
                self.roi_json.set(str(roi_dir / "roi_grid_config.json"))
            # Find and launch roi_select_gui.py
            candidates = [
                roi_dir / "roi_select_gui.py",
                Path.cwd() / "roi_select_gui.py"
            ]
            for c in candidates:
                if c.exists():
                    subprocess.Popen([sys.executable, str(c)], cwd=str(c.parent))
                    self._log(f"Launched old ROI selector: {c}")
                    return
            messagebox.showinfo("Not found", "Could not find 'roi_select_gui.py'. Place it next to the ROI JSON folder or in the current directory.")
        except Exception as e:
