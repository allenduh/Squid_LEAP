...
class ROIGridUI:
    ...
    def _on_click(self, e):
        ...
        if self.corners is not None:
            # Distance-based hit test (works 100% regardless of picker)
            pts = self.corners
            dists = np.hypot(pts[:,0]-x, pts[:,1]-y)
            tol = max(6.0, 0.01*max(self.img.shape[0], self.img.shape[1]))  # data-space tolerance
            i = int(np.argmin(dists))
            if dists[i] <= tol:
                self.drag_idx = i
                return
        # otherwise collect four clicks
    ...
    def _order_corners(points: List[Tuple[float,float]]):
        ...
        # find TL (smallest y then x), rotate so TL first → TL, TR, BR, BL
...
# ============================== original v3 binning pipeline (unchanged) ==============================
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("RAW → (Binned TIFF + NPZ) [v3 + ROI v2]")
