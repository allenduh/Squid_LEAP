...
void CThreadWorker::PutObjectToQueueIn(void* f)
{
    ...
    if(countQueueInMax < countQueueIn) countQueueInMax = countQueueIn;
}

// interface to extenal
/*
  Get multiple frames from queue to driver. 
  We don't know how many items in this queue. It could be more than FRAMES_PER_SAVER although 
  we check this number when calling PutFrameToQueueToSave().
  So count points to the size of f when passed in. Value changed to the actual number when
  function returns.
  f is a pointer to pointes arrary
*/
...
{
    LOG_LINE("Child Thread Start: %s", threadName);
    if(WorkerThreadStart()) {
        while(IsMachineOn()) {
...
#if defined(__GNUC__)
                CEvtUtil::SleepMicroSeconds(interval);
#else
                CEvtUtil::SLEEP(intervalMilliSeconds);
#endif
            }
        }
    }

    LOG_LINE("Child Thread DONE: %s", threadName);
}
