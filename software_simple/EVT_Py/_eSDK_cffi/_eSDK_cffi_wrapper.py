...
class _EvtCamera:
    ...
    def allocate_frame(self, width: int, height: int, pixel_type: int) -> _EvtFrame:
        if width == 0:
    ...
    def get_frame(self, timeout: int) -> _EvtFrame:
        frame_ffi = _g_ffi.new("EmergentFrame*")
        _call_evt_ffi("EVT_CameraGetFrame", self._handle, frame_ffi, timeout)
    ...
    def get_enum_range(self, param: str) -> str:
        name_ffi = _g_ffi.new("char[]", bytes(param, "ascii"))
        enum_str_buffer_ffi = _g_ffi.new("char[]", _ENUM_STR_MAX_SIZE)
        _call_evt_ffi("EVT_CameraGetEnumParamRange", self._handle, name_ffi, enum_str_buffer_ffi, _ENUM_STR_MAX_SIZE, _g_ffi.NULL)
        range = _g_ffi.string(enum_str_buffer_ffi).decode("ascii")
        return [val for val in range.split(',') if val]
...
class _EvtDeviceInfo:
    def __init__(self, camera_id: str, mac_address: str, ip_address: str, dev_idx: int, dev_info_set_wrapper: _EvtDeviceInfoSetWrapper):
...
class _EvtContext:
    def __init__(self):
        ...
        if os_name == "nt":
            os.add_dll_directory(Path(os.environ['EMERGENT_DIR']) / Path("eSDK/GenICam/bin/Win64_x64"))
            os.add_dll_directory(Path(os.environ['EMERGENT_DIR']) / Path("eSDK/bin"))
            os.add_dll_directory(Path(os.environ['EMERGENT_DIR']) / Path("eSDK/ffmpeg/bin"))
    ...
    def list_devices(self) -> _EvtDeviceList:
        ...
        for dev_idx in range(dev_info_set_ffi.countDevice):
            ...
            mac_address = _g_ffi.string(dev_info_ffi.macAddress).decode("ascii")
            ip_address = _g_ffi.string(dev_info_ffi.currentIp).decode("ascii")

            dev_infos.append(_EvtDeviceInfo(camera_id, mac_address, ip_address, dev_idx, dev_info_set_wrapper))
