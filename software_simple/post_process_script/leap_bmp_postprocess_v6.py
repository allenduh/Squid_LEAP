#!/usr/bin/env python3
# LEAP BMP post-processor v6
# What's new vs v5/v4:
# - Page 1: Show ONLY ROI boxes on the raw image (no traces). This makes later trace-only views "pop".
# - Page 2: Trace-only overlay, cropped to the ROI array region with a small boundary so traces are larger. No ROI boxes.
# - Page 3: Percent-normalized small multiples with P25/P75 red dotted lines (two-level view to judge sensitivity).
# - Page 4: Full-frame DIFFERENCE image (frame@P75 minus frame@P25), cropped to the ROI-array region; 'RdBu' with 0 -> white.
# - Page 5: Full-frame PERCENT-CHANGE image: (High-Low) / ((High+Low)/2); same crop and colormap as Page 4.
#
# Notes:
# - leap_roi_tools.py is unchanged.
# - We choose robust global frames for P25 and P75 by taking the MEDIAN of the nearest-frame indices across all ROIs.
...
def compute_roi_crop(centers: np.ndarray, box: int, H: int, W: int, margin_frac: float = 0.5):
    """Compute a crop window around the ROI grid (xmin,xmax,ymin,ymax) with a small boundary in pixels."""
    h = box // 2
    xs = centers[:, 0]
    ys = centers[:, 1]
    xmin = max(0, int(xs.min() - h))
    xmax = min(W - 1, int(xs.max() + h))
    ymin = max(0, int(ys.min() - h))
    ymax = min(H - 1, int(ys.max() + h))
    # add margin
    margin = int(round(box * margin_frac))
...
def nearest_frame_index(trace: np.ndarray, target: float) -> int:
    """Index of sample in 'trace' whose value is nearest to 'target'."""
    idx = int(np.argmin(np.abs(np.asarray(trace, dtype=np.float32) - float(target))))
    return idx

# ----------------------------- PDF rendering --------------------------------

def render_pdf_pages_v6(first_frame: np.ndarray, centers: np.ndarray, box: int,
                        ...
                        bmp_list: List[Path]):
    """Five pages: (1) image + ROI boxes; (2) trace-only overlay (cropped); 
    (3) percent-normalized small multiples with P25/P75; 
    (4) crop full-frame difference (P75-P25); 
    (5) percent change image (diff / avg)."""
    H, W = first_frame.shape
    B, T = traces.shape
    h = box // 2

    # Crop bounds for ROI array region
    xmin, xmax, ymin, ymax = compute_roi_crop(centers, box, H, W, margin_frac=0.5)

    # Decide robust global frames for "low" (near P25) and "high" (near P75)
    low_idxs = np.zeros(B, dtype=np.int32)
    high_idxs = np.zeros(B, dtype=np.int32)
    for b in range(B):
        low_idxs[b] = nearest_frame_index(traces[b], p25s[b])
        high_idxs[b] = nearest_frame_index(traces[b], p75s[b])
    f_low = int(np.median(low_idxs))
    f_high = int(np.median(high_idxs))
    f_low = np.clip(f_low, 0, T-1)
    f_high = np.clip(f_high, 0, T-1)

    #f_low = 600
    #f_high = 700

    # Load those two frames
    frame_low = load_bmp_uint8(bmp_list[f_low])
    frame_high = load_bmp_uint8(bmp_list[f_high])

    # Differences
    diff = frame_high.astype(np.float32) - frame_low.astype(np.float32)
    avg = (frame_high.astype(np.float32) + frame_low.astype(np.float32)) / 2.0
    with np.errstate(divide='ignore', invalid='ignore'):
        pct_change = np.where(avg != 0, diff / avg, 0.0)

    # Robust limits for visualization (avoid outliers): use 1st/99th percentiles in cropped region
    c_diff = diff[ymin:ymax+1, xmin:xmax+1]
    lo, hi = np.percentile(c_diff, [1, 99])
    lim = max(abs(lo), abs(hi))
    if not np.isfinite(lim) or lim <= 0:
        lim = np.max(np.abs(c_diff)) if c_diff.size > 0 else 1.0
        lim = lim if np.isfinite(lim) and lim > 0 else 1.0

    c_pct = pct_change[ymin:ymax+1, xmin:xmax+1]
    finite_mask = np.isfinite(c_pct)
    if np.any(finite_mask):
        lo_p, hi_p = np.percentile(c_pct[finite_mask], [1, 99])
    else:
        lo_p, hi_p = (-0.01, 0.01)
    lim_p = max(abs(lo_p), abs(hi_p))
    if not np.isfinite(lim_p) or lim_p <= 0:
        lim_p = 0.01

    with PdfPages(out_pdf) as pdf:
        # ---------------- Page 1: raw image + ROI boxes ONLY ----------------
        ...
        # ---------------- Page 2: trace-only overlay (CROPPED), no boxes ----
        fig2, ax2 = plt.subplots(figsize=((xmax-xmin+1)/100, (ymax-ymin+1)/100), dpi=100)
        ax2.imshow(first_frame[ymin:ymax+1, xmin:xmax+1], cmap="gray")
        ax2.set_axis_off()
        # Use a bright colormap so traces are visible on gray
        cmap = plt.cm.inferno
        ...
        for b in range(B):
            ...
            # Map to cropped coords
            y_ext0 = max(ymin, int(cy - box))  # vertical span to draw
            y_ext1 = min(ymax, int(cy + box))
            ys_line = (cy - tr_norm * (y_ext1 - y_ext0) * 0.8) - ymin
            # Horizontal stretch across ~2.5*box for visibility
            roi_w = box
            stretch = max(1, int(round(2.5 * roi_w)))
            xL = int(cx - stretch/2); xR = int(cx + stretch/2)
            xL = max(xmin, xL); xR = min(xmax, xR)
            xs_line = np.linspace(xL - xmin, xR - xmin, num=len(tr))
            ax2.plot(xs_line, ys_line, linewidth=1.0, color=cmap(norm(deltas[b])))
        mappable = plt.cm.ScalarMappable(norm=norm, cmap=cmap)
        cbar = fig2.colorbar(mappable, ax=ax2, fraction=0.046, pad=0.02)
        cbar.set_label("Δ = P75−P25 (intensity)")
        fig2.tight_layout(pad=0)
        pdf.savefig(fig2, bbox_inches="tight", pad_inches=0)
        plt.close(fig2)

        # ---------------- Page 3: percent-normalized small multiples --------
        fig3 = plt.figure(figsize=(cols*2.5, rows*1.9), dpi=160)
        gs3 = GridSpec(rows, cols, figure=fig3, wspace=0.25, hspace=0.35)
        for b in range(B):
            r = b // cols; c = b % cols
        ...
        # ---------------- Page 4: full-frame difference (cropped) -----------
        fig4, ax4 = plt.subplots(figsize=((xmax-xmin+1)/100, (ymax-ymin+1)/100), dpi=100)
        norm_diff = TwoSlopeNorm(vcenter=0.0, vmin=-lim, vmax=lim)
        im4 = ax4.imshow(c_diff, cmap="RdBu_r", norm=norm_diff)
        ax4.set_axis_off()
        cbar4 = fig4.colorbar(im4, ax=ax4, fraction=0.046, pad=0.02)
        cbar4.set_label(f"ΔI = I@P75 − I@P25 (a.u.) | frames: low={f_low}, high={f_high}")
        fig4.tight_layout(pad=0)
        pdf.savefig(fig4, bbox_inches="tight", pad_inches=0)
        plt.close(fig4)

        # ---------------- Page 5: percent-change image (cropped) ------------
        fig5, ax5 = plt.subplots(figsize=((xmax-xmin+1)/100, (ymax-ymin+1)/100), dpi=100)
        norm_pct = TwoSlopeNorm(vcenter=0.0, vmin=-lim_p, vmax=lim_p)
        im5 = ax5.imshow(c_pct, cmap="RdBu_r", norm=norm_pct)
        ax5.set_axis_off()
        cbar5 = fig5.colorbar(im5, ax=ax5, fraction=0.046, pad=0.02)
        cbar5.set_label("Percent change = (High − Low) / ((High + Low)/2)")
        fig5.tight_layout(pad=0)
        pdf.savefig(fig5, bbox_inches="tight", pad_inches=0)
        plt.close(fig5)
...
def process_experiment(exp_dir: str | Path, rows: int = 8, cols: int = 9, box: int = 32, fps: float = 5000.0,
    ...
    # PDF
    pdf_path = exp_dir / f"{exp_dir.name}_report_v6.pdf"
    render_pdf_pages_v6(first_frame, centers, box, traces, fps, pdf_path, rows=rows, cols=cols,
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="LEAP BMP-only post-processor (v6)")
