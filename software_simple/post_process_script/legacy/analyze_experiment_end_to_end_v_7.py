...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="LEAP End-to-End V7 (GUI-first, ROI from V6-style)")
    ...
    # --init-quad can be "auto" or four pairs "x1,y1;x2,y2;x3,y3;x4,y4"
    p.add_argument("--init-quad", type=str, default="", help='Seed TL,TR,BL,BR. "auto" or "x1,y1;x2,y2;x3,y3;x4,y4"')
...
class FourPointSelector:
    ...
    def __init__(self, frame: np.ndarray, init_points: Optional[np.ndarray]=None):
        ...
        # NEW: seed visible handles if provided
...
def run_with_gui(argv: Optional[List[str]] = None):
    args = parse_args(argv)

    # Either take --exp or ask via picker
    if args.exp:
    ...
    params = dict(rows=int(args.rows), cols=int(args.cols), box=int(args.box), fps=float(args.fps),
                  raw_h=608, raw_w=1024, frames_per_file=10,
                  raw_dtype='uint8', endianness='little',
                  header_bytes=0, row_stride_bytes=0,
                  invoke_plotter=True)

    # (Only show the TK param dialog if you want interactive overrides and no CLI was used for geometry)
    # If you still want the dialog, keep your existing ParamDialog block here conditionally.
    ...
    if has_bmps:
        ...
        # (keep your RAW first-frame preview code as-is)
        # ...
        # set "first" to the preview frame

    # Build initial quad
    init_quad = None
    if args.init_quad:
        if args.init_quad.strip().lower() == "auto":
            inset = 10  # px inset from borders (tweak if you like)
            init_quad = np.array([
                [inset, inset],           # TL
                [W - inset, inset],       # TR
                [inset, H - inset],       # BL
            ...
            try:
                pairs = [p.strip() for p in args.init_quad.split(";")]
                if len(pairs) != 4: raise ValueError
                xy = []
                for p in pairs:
                    x, y = p.split(",")
                    xy.append([float(x), float(y)])
                init_quad = np.array(xy, dtype=np.float32)
            except Exception:
                print("[WARN] --init-quad format invalid. Expected 'auto' or 'x1,y1;x2,y2;x3,y3;x4,y4'")

    # 4-point selection (now with optional seeds)
    fp_selector = FourPointSelector(first, init_points=init_quad)
    quad = fp_selector.go()  # TL, TR, BL, BR

    # Grid adjuster and the rest stays the same, but use args/params directly
