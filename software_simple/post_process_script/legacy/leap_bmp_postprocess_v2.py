...
#!/usr/bin/env python3
"""
leap_bmp_postprocess_v2.py
- Clean BMP-only pipeline that imports FourPointSelector & GridAdjuster from leap_roi_tools
- Keeps frames as uint8 to reduce memory
- Saves exp_block_data.npz
- Exports a PDF visual report:
    * First frame as background
    * For each grid box, overlay the raw trace (superposed in the box region)
- Opens the PDF after saving

CLI:
  python leap_bmp_postprocess_v2.py --exp "D:\path\to\experiment" --rows 8 --cols 9 --box 32 --fps 5000 --init-quad auto
"""
...
def render_pdf(first_frame: np.ndarray, centers: np.ndarray, box: int,
               traces: np.ndarray, fps: float, out_pdf: Path):
    """
    Make a single-page PDF:
    - grayscale first frame as background
    - in each grid box, overlay the raw trace scaled to the box rectangle
    """
    H, W = first_frame.shape
    fig, ax = plt.subplots(figsize=(W/100, H/100), dpi=100)  # keep 1:1 pixels
    ...
    for b in range(B):
        cx, cy = centers[b]
        x0, x1 = int(cx - h), int(cx - h + box)
        y0, y1 = int(cy - h), int(cy - h + box)
        ...
        tr_norm = (tr - mn) / span  # 0..1

        xs = np.linspace(x0 + 1, x1 - 1, num=T)
        ys = y1 - 1 - tr_norm * (y1 - y0 - 2)  # top-down image coords

        ax.plot(xs, ys, linewidth=0.8)
        ax.add_patch(Rectangle((x0, y0), x1-x0, y1-y0, fill=False, linewidth=0.5))
...
def process_experiment(exp_dir: str | Path, rows: int = 8, cols: int = 9, box: int = 32, fps: float = 5000.0,
    ...
    # Save NPZ (first_frame uint8; traces float32)
    payload = dict(
        experiment_dir=str(exp_dir),
        first_frame_path=str(bmp_list[0]),
    ...
    # PDF report
    pdf_path = exp_dir / "exp_report.pdf"
    render_pdf(first_frame, centers, box, traces, fps, pdf_path)
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(description="LEAP BMP-only post-processor (clean)")
